...
class PlaylistLoader extends EventHandler {
  private loaders: Partial<
    Record<PlaylistContextType, Loader<PlaylistLoaderContext>>
  > = {};
  ...
  getInternalLoader(
    context: PlaylistLoaderContext,
  ): Loader<PlaylistLoaderContext> | undefined {
  ...
  load(context: PlaylistLoaderContext): boolean {
    ...
    logger.debug(
      `Loading playlist of type ${context.type}, level: ${context.level}, id: ${context.id}`,
    );
  ...
  _handleNetworkError(
    ...
    response: LoaderResponse | null = null,
  ) {
    logger.info(
      `A network error occured while loading a ${context.type}-type playlist`,
    );
