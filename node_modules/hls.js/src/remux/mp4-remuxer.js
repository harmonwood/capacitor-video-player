...
class MP4Remuxer {
  ...
  remux(
    ...
    if (this.ISGenerated) {
      ...
      if (nbAudioSamples && nbVideoSamples) {
        ...
        const startPTS = videoTrack.samples.reduce(
          (minPTS, sample) => Math.min(minPTS, sample.pts),
  ...
  generateIS(audioTrack, videoTrack, timeOffset) {
    ...
    if (videoTrack.sps && videoTrack.pps && videoSamples.length) {
      ...
      if (computePTSDTS) {
        const startPTS = videoSamples.reduce(
          (minPTS, sample) => Math.min(minPTS, sample.pts),
          videoSamples[0].pts,
        );
  ...
  remuxVideo(track, timeOffset, contiguous, audioTrackLength) {
    ...
    if (ptsDtsShift < 0) {
      if (ptsDtsShift < averageSampleDuration * -2) {
        ...
        logger.warn(
          `PTS < DTS detected in video samples, offsetting DTS to PTS ${toMsFromMpegTsClock(
      ...
      if (i < nbSamples - 1) {
        ...
        if (config.stretchShortVideoTrack) {
          ...
          if (deltaToFrameEnd > gapTolerance) {
            ...
            logger.log(
              `It is approximately ${toMsFromMpegTsClock(
                deltaToFrameEnd,
                false,
              )} ms to the next segment; using duration ${toMsFromMpegTsClock(
                mp4SampleDuration,
                false,
          ...
          if (contiguous) {
            logger.warn(
              `Dropping 1 audio frame @ ${
                toMsFromMpegTsClock(nextPts, true) / 1000
              }s due to ${toMsFromMpegTsClock(delta, true)} ms overlap.`,
            );
            ...
            logger.warn(
              `Audio frame @ ${
                toMsFromMpegTsClock(pts, true) / 1000
              }s overlaps nextAudioPts by ${toMsFromMpegTsClock(
                delta,
                true,
              )} ms.`,
            );
        ...
        // 3: currentTime (aka nextPtsNorm) is not 0
          ...
          logger.warn(
            `Injecting ${missing} audio frames @ ${
              toMsFromMpegTsClock(nextPts, true) / 1000
            }s due to ${toMsFromMpegTsClock(delta, true)} ms gap.`,
          );
        ...
        if (contiguous && track.isAAC) {
          ...
          if (delta) {
            if (delta > 0 && delta < MAX_SILENT_FRAME_DURATION_90KHZ) {
              ...
              logger.log(
                `drop overlapping AAC sample, expected/parsed/delta: ${toMsFromMpegTsClock(
                  nextAudioPts,
                  true,
                )} ms / ${toMsFromMpegTsClock(
                  pts,
                  true,
                )} ms / ${toMsFromMpegTsClock(-delta, true)} ms`,
              );
  ...
  remuxEmptyAudio(track, timeOffset, contiguous, videoData) {
    ...
    // silent frame
    let silentFrame = AAC.getSilentFrame(
      track.manifestCodec || track.codec,
      track.channelCount,
    );
