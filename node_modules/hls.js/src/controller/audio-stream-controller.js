...
class AudioStreamController extends BaseStreamController {
        ...
        if ((bufferLen < maxBufLen || audioSwitch) && trackId < tracks.length) {
          ...
          // if bufferEnd before start of playlist, load first fragment
            ...
            if (bufferEnd < end) {
              ...
              if (
                ...
                foundFrag = BinarySearch.search(fragments, frag =>
                  fragmentWithinToleranceTest(
                    bufferEnd,
                    maxFragLookUpTolerance,
                    frag,
          ...
          if (frag) {
            ...
            if (frag.encrypted) {
              ...
              if (
                audioSwitch ||
                this.fragmentTracker.getState(frag) === FragmentState.NOT_LOADED
              ) {
                logger.log(
      ...
      case State.WAITING_INIT_PTS:
        ...
        if (waitingFrag) {
          ...
          if (this.initPTS[waitingFragCC] !== undefined) {
            ...
            logger.log(
              `Waiting fragment cc (${waitingFragCC}) cancelled because video is at cc ${this.videoTrackCC}`,
            );
            ...
            if (waitingFragmentAtPosition < 0) {
              logger.log(
                `Waiting fragment cc (${waitingFragCC}) @ ${waitingFrag.frag.start} cancelled because another fragment at ${bufferInfo.end} is needed`,
              );
              this.clearWaitingFragment();
            }
          }
  ...
  onAudioTrackLoaded(data) {
    ...
    logger.log(
      `track ${trackId} loaded [${newDetails.startSN},${newDetails.endSN}],duration:${duration}`,
    );
      ...
      if (this.startPosition === -1) {
        ...
        if (Number.isFinite(startTimeOffset)) {
          ...
          if (newDetails.live) {
            ...
            logger.log(
              `compute startPosition for audio-track to ${this.startPosition}`,
            );
          } else {
  ...
  onFragLoaded(data) {
    ...
    if (
      ...
      let track = this.tracks[this.trackId],
        ...
        cc = fragCurrent.cc,
        audioCodec =
          this.config.defaultAudioCodec || track.audioCodec || 'mp4a.40.2',
      ...
      if (sn === 'initSegment') {
        ...
        if (initPTS !== undefined) {
          ...
          logger.log(
            `Demuxing ${sn} of [${details.startSN} ,${details.endSN}],track ${trackId}`,
          );
          ...
          logger.log(
            `Unknown video PTS for cc ${cc}, waiting for video PTS before demuxing audio frag ${sn} of [${details.startSN} ,${details.endSN}],track ${trackId}`,
          );
  ...
  onFragParsingInitSegment(data) {
    ...
    if (
      ...
      if (track) {
        ...
        logger.log(
          `audio track:audio,container:${track.container},codecs[level/parsed]=[${track.levelCodec}/${track.codec}]`,
        );
    ...
    switch (data.details) {
      ...
      case ErrorDetails.FRAG_LOAD_TIMEOUT:
        ...
        if (!data.fatal) {
          ...
          if (loadError <= config.fragLoadingMaxRetry) {
            ...
            logger.error(
              `AudioStreamController: ${data.details} reaches max retry, redispatch as fatal ...`,
            );
      ...
      case ErrorDetails.KEY_LOAD_TIMEOUT:
        ...
        if (this.state !== State.ERROR) {
          ...
          logger.warn(
            `AudioStreamController: ${data.details} while loading frag, now switching to ${this.state} state ...`,
          );
        }
        break;
        ...
        if (
          ...
          if (mediaBuffered) {
            ...
            if (config.maxMaxBufferLength >= config.maxBufferLength) {
              ...
              logger.warn(
                `AudioStreamController: reduce max buffer length to ${config.maxMaxBufferLength}s`,
              );
            }
            this.state = State.IDLE;
          } else {
