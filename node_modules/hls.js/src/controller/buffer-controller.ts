...
class BufferController extends EventHandler {
  ...
  private _onSBUpdateEnd = () => {
    ...
    if (this.audioTimestampOffset && this.sourceBuffer.audio) {
      ...
      logger.warn(
        `change mpeg audio timestamp offset from ${audioBuffer.timestampOffset} to ${this.audioTimestampOffset}`,
      );
    ...
    let pending = this.segments.reduce(
      (counter, segment) => (segment.parent === parent ? counter + 1 : counter),
      0,
    );
    ...
    for (let streamType in sbSet) {
      ...
      if (!sb) {
        throw Error(
          `handling source buffer update end error: source buffer for ${streamType} uninitilized and unable to update buffered TimeRanges.`,
        );
      }
  ...
  createSourceBuffers(tracks: TrackSet) {
    ...
    for (let trackName in tracks) {
      if (!sourceBuffer[trackName]) {
        ...
        try {
          let sb = (sourceBuffer[trackName] = mediaSource.addSourceBuffer(
            mimeType,
          ));
  ...
  flushLiveBackBuffer() {
    ...
    const targetBackBufferPosition =
      currentTime - Math.max(liveBackBufferLength, this._levelTargetDuration);
  ...
  onLevelUpdated({
    ...
    if (details.fragments.length > 0) {
      ...
      this._levelTargetDuration =
        details.averagetargetduration || details.targetduration || 10;
  ...
  updateMediaElementDuration() {
    ...
    if (this._live === true && config.liveDurationInfinity === true) {
      ...
      logger.log(
        `Updating Media Source duration to ${this._levelDuration.toFixed(3)}`,
      );
      ...
      if (err.code === 22) {
        ...
        */
        if (this.appendError > config.appendErrorMaxRetry) {
          logger.log(
            `fail ${config.appendErrorMaxRetry} times to append segment in sourceBuffer`,
          );
          this.segments = [];
          event.fatal = true;
        }
  ...
  */
  flushBuffer(
    ...
    logger.log(
      `flushBuffer,pos/start/end: ${currentTime}/${startOffset}/${endOffset}`,
    );
      ...
      for (let i = 0; i < sb.buffered.length; i++) {
        ...
        */
        if (Math.min(removeEnd, bufEnd) - removeStart > 0.5) {
          ...
          logger.log(
            `sb remove ${type} [${removeStart},${removeEnd}], of [${bufStart},${bufEnd}], pos:${currentTime}`,
          );
