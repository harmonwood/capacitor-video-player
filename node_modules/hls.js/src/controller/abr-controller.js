...
class AbrController extends EventHandler {
  ...
  _abandonRulesCheck() {
    ...
    */
    ...
    if (
      video &&
      stats &&
      ((!video.paused && video.playbackRate !== 0) || !video.readyState) &&
      frag.autoLevel &&
      frag.level
    ) {
      ...
      if (requestDelay > (500 * frag.duration) / playbackRate) {
        ...
        const loadRate = Math.max(
          1,
          stats.bw ? stats.bw / 8 : (stats.loaded * 1000) / requestDelay,
        ...
        const levelBitrate = level.realBitrate
          ? Math.max(level.realBitrate, level.bitrate)
          : level.bitrate;
        const expectedLen = stats.total
          ? stats.total
          : Math.max(
              stats.loaded,
        ...
        const bufferStarvationDelay =
          (BufferHelper.bufferInfo(video, pos, hls.config.maxBufferHole).end -
            pos) /
          playbackRate;
        ...
        if (
          ...
          for (
            ...
            const fragLevelNextLoadedDelay =
              (frag.duration * levelNextBitrate) / (8 * 0.8 * loadRate);
          ...
          if (fragLevelNextLoadedDelay < fragLoadedDelay) {
            logger.warn(
              `loading too slow, abort fragment loading and switch to level ${nextLoadLevel}:fragLoadedDelay[${nextLoadLevel}]<fragLoadedDelay[${
                frag.level - 1
              }];bufferStarvationDelay:${fragLevelNextLoadedDelay.toFixed(
                1,
              )}<${fragLoadedDelay.toFixed(1)}:${bufferStarvationDelay.toFixed(
                1,
              )}`,
            );
  ...
  get _nextABRAutoLevel() {
    ...
    const currentFragDuration = this.fragCurrent
    ...
    const playbackRate =
      video && video.playbackRate !== 0 ? Math.abs(video.playbackRate) : 1.0;
    const avgbw = this._bwEstimator
      ? this._bwEstimator.getEstimate()
      : config.abrEwmaDefaultEstimate;
    // bufferStarvationDelay is the wall-clock time left until the playback buffer is exhausted.
    const bufferStarvationDelay =
      (BufferHelper.bufferInfo(video, pos, config.maxBufferHole).end - pos) /
      playbackRate;
  ...
  _findBestLevel(
    ...
    for (let i = maxAutoLevel; i >= minAutoLevel; i--) {
      ...
      const avgDuration = levelDetails
