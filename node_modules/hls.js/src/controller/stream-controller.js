...
class StreamController extends BaseStreamController {
  ...
  _findFragment(
    ...
    if (fragNextLoad) {
      ...
      const sameLevel =
        fragPreviousLoad && fragNextLoad.level === fragPreviousLoad.level;
      ...
      if (fragPreviousLoad && fragNextLoad.sn === fragPreviousLoad.sn) {
        if (sameLevel && !fragNextLoad.backtracked) {
          ...
          if (nextSnFrag && nextSnFrag.backtracked) {
            logger.warn(
              `Already backtracked from fragment ${nextSnFrag.sn}, will not backtrack to fragment ${fragNextLoad.sn}. Loading fragment ${nextSnFrag.sn}`,
            );
            fragNextLoad = nextSnFrag;
          } else {
  ...
  _loadKey(frag, levelDetails) {
    logger.log(
      `Loading key for ${frag.sn} of [${levelDetails.startSN} ,${levelDetails.endSN}],level ${this.level}`,
    );
    ...
    if (
      ...
      logger.log(
        `Loading ${frag.sn} of [${levelDetails.startSN} ,${
          levelDetails.endSN
        }],level ${this.level}, currentTime:${pos.toFixed(
          3,
  ...
  */
  ...
  onFragLoaded(data) {
    ...
    if (
      ...
      logger.log(
        `Loaded ${fragCurrent.sn} of [${details.startSN} ,${details.endSN}],level ${fragCurrent.level}`,
      );
      if (fragLoaded.bitrateTest && hls.nextLoadLevel) {
        ...
        logger.log(
          `Parsing ${fragCurrent.sn} of [${details.startSN} ,${details.endSN}],level ${fragCurrent.level}, cc ${fragCurrent.cc}`,
        );
        ...
        const accurateTimeOffset =
          !(media && media.seeking) && (details.PTSKnown || !details.live);
        const initSegmentData = details.initSegment
    ...
    if (
      ...
      logger.log(
        `Parsed ${data.type},PTS:[${data.startPTS.toFixed(
          ...
          3,
        )}/${data.endDTS.toFixed(3)}],nb:${data.nb},dropped:${
          data.dropped || 0
        }`,
      );
      ...
      let drift = LevelHelper.updateFragPTSDTS(
          level.details,
          frag,
          data.startPTS,
          data.endPTS,
      ...
      case ErrorDetails.KEY_LOAD_TIMEOUT:
        if (!data.fatal) {
          ...
          if (this.fragLoadError + 1 <= this.config.fragLoadingMaxRetry) {
            ...
            logger.error(
              `mediaController: ${data.details} reaches max retry, redispatch as fatal ...`,
            );
      ...
      case ErrorDetails.LEVEL_LOAD_TIMEOUT:
        if (this.state !== State.ERROR) {
          if (data.fatal) {
            ...
            this.state = State.ERROR;
            logger.warn(
              `streamController: ${data.details},switch to ${this.state} state ...`,
            );
          } else {
  ...
  onBufferFlushed() {
    ...
    */
    ...
    if (media) {
      ...
      const elementaryStreamType = this.audioOnly
  ...
  _seekToStartPos() {
    ...
    if (currentTime !== startPosition && startPosition >= 0) {
      ...
      logger.log(
        `seek to target start position ${startPosition} from current time ${currentTime}. ready state ${media.readyState}`,
      );
