  ...
  recoverLevel(errorEvent, levelIndex, levelError, fragmentError) {
    ...
    if (levelError) {
      if (this.levelRetryCount + 1 <= config.levelLoadingMaxRetry) {
        ...
        delay = Math.min(
          Math.pow(2, this.levelRetryCount) * config.levelLoadingRetryDelay,
        ...
        logger.warn(
          `level controller, ${errorDetails}, retry in ${delay} ms, current retry count is ${this.levelRetryCount}`,
        );
      } else {
        logger.error(
    ...
    if (levelError || fragmentError) {
      ...
      if (redundantLevels > 1 && level.loadError < redundantLevels) {
        ...
        logger.warn(
          `level controller, ${errorDetails} for level ${levelIndex}: switching to redundant URL-id ${level.urlId}`,
        );
